...
def get_phicov(f, g, fx, gx, i):

    phi = np.trace(((f.T @ fx[i]) / f.shape[0]) @ ((g.T @ gx[i]) / g.shape[0]))
    ...
    for m in range(f.shape[0]):
        for n in range(g.shape[0]):
            if fx[i][m].T @ gx[i][n] > -1:
                phi_cov += (f[m].T @ g[n])*(f[m].T @ g[n]) * (1 + (fx[i][m].T @ gx[i][n]))
                nd += (f[m].T @ g[n]) * (1 + (fx[i][m].T @ gx[i][n]))
    phi_cov = phi_cov / f.shape[0] / g.shape[0] - ((nd / f.shape[0] / g.shape[0])**2)
    print(phi, '\t', phi_cov)
    return phi, phi_cov


def alpha(phi, phi_x, phi_cov_x, dim, last_alpha):
    "get optimized alpha from numerical solution"
    try:
        A = np.zeros((dim, dim))
        A[0][0] = 1/6 * phi_cov_x[0] / phi_x[0]
        for i in range(1, dim):
            for j in range(1, dim):
                A[i][j] = (phi_x[i] - phi_x[0]) * (phi_x[j] - phi_x[0]) / phi
        ...
        prob = cvx.Problem(obj, [np.ones((1, dim)) @ alphav== 1., np.eye(dim) @ alphav >= np.zeros(dim)])
        prob.solve()
...
if __name__ == "__main__":
    ...
    for eo in range(epoch_out):
        ...
        alpha_m = alpha(phi, phi_x, phi_cov_x, DIM, alpha0)
        # ============================================================
